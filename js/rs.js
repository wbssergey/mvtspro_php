...
(
	rs.guicontainer,
	{
		...
		{
			//alert(this.dom.table_rowset.rows[2]);
	/*		var output = '';
			var oz=0;
			for (property in this.dom.table_rowset.rows[2]) {
				oz++;
				if(oz > 15) {
			  output += '@'+property + ': ' + this.dom.table_rowset.rows[2][property]+'; ';
				};
			}
			alert(this.dom.table_rowset.rows[2]['innerHTML']);
//<td class=" ctl-lookup"></td><td class=" ctl-lookup"></td><td class=" ctl-lookup"></td><td class=" ctl-lookup"></td><td class=" ctl-lookup"></td><td>
*/
//			alert(this.dom.table_rowset.rows[6]['innerHTML']);
	...
	//		var pts2='-select-';
			...
			for (var i=0; i<this.rowset.length; i++)
			{
				...
				vpts=this.dom.table_rowset.rows[i+2]['innerHTML'];
			
				var bpts=vpts.indexOf(pts) > 0;
			//	if(vpts.indexOf(pts2)>0) {     // this.dom.table_rowset.rows is readonly
				//	this.dom.table_rowset.rows[i+2]['innerHTML'].replace('-select-','');
			//	}
				if(bpts||this.illuminateRows[i])
				{
